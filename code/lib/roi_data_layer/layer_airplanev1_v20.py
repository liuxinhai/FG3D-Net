...
from roi_data_layer.minibatch_airplane_v20 import get_minibatch
import numpy as np
import random
...
class RoIDataLayer(object):
    ...
    def __init__(self, roidb, num_classes):
        ...
        self.views = 20
        self._num_models = len(self._roidb) / self.views
        self._shuffle_roidb_inds()


        self._classnum = [955, 24, 127, 186, 545, 549, 288, 390, 35, 201, 15, 101, 25]
    ...
    def _get_next_minibatch_inds(self):
        ...
        if cfg.TRAIN.HAS_RPN:
            ...
            db_inds = [self._perm[self._cur]*self.views+self._shift]
            self._shift = self._shift + 1

            if self._shift % self.views == 0:
