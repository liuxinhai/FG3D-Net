...
from roi_data_layer.minibatch_mn40 import get_minibatch
import numpy as np
import random
...
class RoIDataLayer(object):
    ...
    def __init__(self, roidb, num_classes):
        ...
        self._classnum = [626,106,515,173,572,335,64,197,889,167,79,138,200,109,200,149,171,155,145,124,149,284,465,200,88,231,240,104,115,128,680,124,90,392,163,344,267,475,87,103]

        self._rand_nums = []
        for i in range(sum(self._classnum)):
            self._rand_nums.append(random.randint(0, 11))

        for i in range(len(self._classnum)):
            temp = [1.0 if k == i else 0.0 for k in range(40)]
    ...
    def _shuffle_roidb_inds(self):
        ...
        self._perm = np.random.permutation(np.arange(self._num_models))
        self._cur = 0
        self._shift = 0
    ...
    def rand_target(self):
        return self._rand_nums[self._perm[self._cur]]
