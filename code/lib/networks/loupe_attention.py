...
class NetVLAD(PoolingBaseModel):
  ...
  """
    ...
    def save_weights(self, sess, weights_path="./vlad_weights.npy"):
        data_dict = {}
        for var in self.var_list:
            var_val = sess.run(var)
            data_dict[var.name] = var_val
        np.save(weights_path, data_dict)
    ...
    """
        variable_name = 'cluster_weights'
        variable_key = variable_name + ":0"
        if self.data_dict is None or variable_key not in self.data_dict:
            cluster_weights = tf.get_variable(variable_name,
            ...
            cluster_weights = tf.get_variable(variable_name,
                                              [self.feature_size, self.cluster_size],
                                              initializer=tf.constant_initializer(self.data_dict[variable_key]))
        self.var_list.append(cluster_weights)
        ...
        if self.add_batch_norm:
            ...
            variable_name = 'cluster_biases'
            variable_key = variable_name + ":0"
            if self.data_dict is None or variable_key not in self.data_dict:
                cluster_biases = tf.get_variable(variable_name,
                                                 [self.cluster_size],
                                                 initializer=tf.random_normal_initializer(
                                                     stddev=1 / math.sqrt(self.feature_size)))
            else:
                ...
                cluster_biases = tf.get_variable(variable_name,
                                                 [self.cluster_size],
                                                 initializer=tf.constant_initializer(self.data_dict[variable_key]))
            self.var_list.append(cluster_biases)
        ...
        variable_name = "cluster_weights2"
        variable_key = variable_name + ":0"
        if self.data_dict is None or variable_key not in self.data_dict:
            cluster_weights2 = tf.get_variable(variable_name,
            ...
            cluster_weights2 = tf.get_variable(variable_name,
                                               [1, self.feature_size, self.cluster_size],
                                               initializer=tf.constant_initializer(self.data_dict[variable_key]))
        self.var_list.append(cluster_weights2)
