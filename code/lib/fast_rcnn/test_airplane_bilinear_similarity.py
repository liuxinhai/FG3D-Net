...
from roi_data_layer.layer_airplanev1_test import RoIDataLayer
...
class SolverWrapper(object):
	...
	#view_sums_extend = tf.tile(view_sums, [self.views,1])
        ...
        #view_sums = tf.nn.softmax(view_sums)
        ...
        #outputs = tf.reshape(outputs, [-1, 12, 4096])
        # #outputs = tf.concat(outputs, 1)
        # outputs = tf.reshape(outputs, [-1, 12, self.hidden_size])
        ...
        #
        # model_feature = tf.concat([bof, states], axis=1)
        # #print(model_feature)
        ...
        cls_logits = tf.matmul(view_sums, fc2_w) + fc2_b
        ...
        part_attention = np.zeros([732,12,20,20], np.float32)
        view_attention = np.zeros([732,12,12], np.float32)
        rl1 = [L1_S1_Similarity, L1_S1_Similarity1, L1_S1_Similarity2, L1_S1_Similarity3, L1_S1_Similarity4,
              L1_S1_Similarity5]
        rl2 = [L1_S1_Similarity6, L1_S1_Similarity7, L1_S1_Similarity8, L1_S1_Similarity9, L1_S1_Similarity10,
               L1_S1_Similarity11]
        for iter in range(model_num):
            ...
            # rl = [L1_S1_Similarity,L1_S1_Similarity1,L1_S1_Similarity2,L1_S1_Similarity3,L1_S1_Similarity4,L1_S1_Similarity5]
            timer.tic()
            L1_S0, L1_S1, L1_S2, L1_S3, L1_S4, L1_S5 = sess.run(rl1, feed_dict=feed_dict, options=run_options, run_metadata=run_metadata)
            ...
            timer.toc()
            print(iter)
        sio.savemat('/data/lxh/Models/fine-grained/attention/part_att_air1.mat', {'pt':part_attention})
        sio.savemat('/data/lxh/Models/fine-grained/attention/view_att_air1.mat', {'vt': view_attention})
        ...
        # fid = open('/home/liuxinhai/fine-grained/results/airplane_bs_90_addGRU_4096_output_no_view_attention.txt', 'a+')
        ...
        print('class acc: %f'%(sum(class_acc / classes_num) / 13))
        # fid = open('/home/liuxinhai/fine-grained/results/airplane_bs_class_90_addGRU_4096_output_no_view_attention.txt', 'a+')
        # fid.write('{:.6f}\n'.format(sum(class_acc / classes_num) / 13))
        # fid.close()
