...
from fast_rcnn.config_attention import cfg
import gt_data_layer.roidb_attention as gdl_roidb
import roi_data_layer.roidb_attention as rdl_roidb
from roi_data_layer.layer_modelnet10_test import RoIDataLayer
...
class SolverWrapper(object):
    ...
    def train_model(self, sess, max_iters):
        ...
        netVLAD = lp.NetVLAD(feature_size = self.feature_size, max_samples = 12*49*self.proposal_number, cluster_size= self.cluster_size, output_dim = self.output_dim,gating=False,add_batch_norm=True,is_training=False)
        ...
        part_features = tf.stack([tf.gather(part_features_fc7, tf.range(self.proposal_number)), tf.gather(part_features_fc71, tf.range(self.proposal_number))],axis = 0)
        part_features = tf.concat([part_features, [tf.gather(part_features_fc72, tf.range(self.proposal_number))]], axis=0)
        part_features = tf.concat([part_features, [tf.gather(part_features_fc73, tf.range(self.proposal_number))]], axis=0)
        part_features = tf.concat([part_features, [tf.gather(part_features_fc74, tf.range(self.proposal_number))]], axis=0)
        part_features = tf.concat([part_features, [tf.gather(part_features_fc75, tf.range(self.proposal_number))]], axis=0)
        part_features = tf.concat([part_features, [tf.gather(part_features_fc76, tf.range(self.proposal_number))]], axis=0)
        part_features = tf.concat([part_features, [tf.gather(part_features_fc77, tf.range(self.proposal_number))]], axis=0)
        part_features = tf.concat([part_features, [tf.gather(part_features_fc78, tf.range(self.proposal_number))]], axis=0)
        part_features = tf.concat([part_features, [tf.gather(part_features_fc79, tf.range(self.proposal_number))]], axis=0)
        part_features = tf.concat([part_features, [tf.gather(part_features_fc710, tf.range(self.proposal_number))]], axis=0)
        part_features = tf.concat([part_features, [tf.gather(part_features_fc711, tf.range(self.proposal_number))]], axis=0)
        ...
        class_ac_test = True
        # class_ac_test = False
        class_acc = np.zeros(10, np.float32)
        if class_ac_test == True:
            model_num = 908
            classes_num = [50, 100, 100, 86, 86, 100, 86, 100, 100, 100]
        else:
            model_num = 3991
        ...
        for iter in range(model_num):
            ...
            if np.argmax(test_acc, axis=1)[0] == np.argmax(train_target):
                sums += 1.0
                if iter < sum(classes_num[0:1]):
                    class_acc[0] += 1.0
                elif iter < sum(classes_num[0:2]):
                    class_acc[1] += 1.0
                elif iter < sum(classes_num[0:3]):
                    class_acc[2] += 1.0
                elif iter < sum(classes_num[0:4]):
                    class_acc[3] += 1.0
                elif iter < sum(classes_num[0:5]):
                    class_acc[4] += 1.0
                elif iter < sum(classes_num[0:6]):
                    class_acc[5] += 1.0
                elif iter < sum(classes_num[0:7]):
                    class_acc[6] += 1.0
                elif iter < sum(classes_num[0:8]):
                    class_acc[7] += 1.0
                elif iter < sum(classes_num[0:9]):
                    class_acc[8] += 1.0
                elif iter < sum(classes_num[0:10]):
                    class_acc[9] += 1.0
        ...
        fid = open('/home/liuxinhai/Faster-R-CNN/Faster-RCNN_TF/modelnet10_epoch.txt', 'a+')
        ...
        print('class acc: %f'%(sum(class_acc / classes_num) / 10))
        fid = open('/home/liuxinhai/Faster-R-CNN/Faster-RCNN_TF/modelnet10_epoch_class.txt', 'a+')
        fid.write('{:.6f}\n'.format(sum(class_acc / classes_num) / 10))
        fid.close()
