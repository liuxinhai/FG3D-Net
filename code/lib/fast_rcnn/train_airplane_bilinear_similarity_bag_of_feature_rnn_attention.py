...
class SolverWrapper(object):
    ...
    def train_model(self, sess, max_iters):
        ...
        #######define bag of feature branch################
        bf_branch = bg.SoftDBoW(feature_size=self.feature_size, max_samples=self.views * self.proposal_number,
                                cluster_size=self.cluster_size, output_dim=self.output_dim,
        ...
        bof_back = bf_branch.forward(tf.reshape(part_features,[self.views*self.proposal_number, self.feature_size]))
        bof = tf.nn.l2_normalize(bof_back, 1)
        ...
        fc2_w = tf.get_variable('fc2_w', [(8+1)*self.feature_size, self.classes], dtype=tf.float32,
                                initializer=w_init)
