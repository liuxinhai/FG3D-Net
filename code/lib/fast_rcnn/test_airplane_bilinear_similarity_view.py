...
class SolverWrapper(object):
    ...
    def __init__(self, sess, saver, network,network2,network4,network6,network8, network10, imdb, roidb, output_dir, pretrained_model=None):
    #def __init__(self, sess, saver, network,network4,network8, imdb, roidb, output_dir, pretrained_model=None):
    ...
    def train_model(self, sess, max_iters):
        ...
        # learning matrix 3
        # Matrix_L2_S1 = tf.get_variable('L2_S1', [self.feature_size, self.feature_size],
        #                                initializer=tf.random_normal_initializer(
        #                                    stddev=1 / math.sqrt(self.feature_size * self.feature_size)))
        ...
        #######get model parts #########
        part_features = tf.stack([part_features_fc7, part_features_fc72], axis=0)
        ...
        # print(part_sum11)
        # concat views
        view_parts = tf.concat([part_sum, part_sum2], axis=0)
        ...
        #         self.proposal_number * self.views)
        ...
        fid = open('/home/liuxinhai/fine-grained/results/airplane_bs_92_add6views.txt', 'a+')
        ...
        fid = open('/home/liuxinhai/fine-grained/results/airplane_bs_class_92_add6views.txt', 'a+')
...
def train_net(sess1, network,network2,network4,network6,network8,network10, imdb, roidb, output_dir, pretrained_model=None, max_iters=40000):
    """Train a Fast R-CNN network."""
    ...
    with tf.Session(config=config) as sess:
        sw = SolverWrapper(sess, saver, network,network2,network4,network6,network8,network10, imdb, roidb, output_dir, pretrained_model=pretrained_model)
