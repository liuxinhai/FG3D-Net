...
class SolverWrapper(object):
    ...
    def train_model(self, sess, max_iters):
        ...
        # learning matrix 2
        Matrix_L1_S2 = tf.get_variable('L1_S2', [self.feature_size*4, self.feature_size*4], initializer=tf.random_normal_initializer(
                                           stddev=1 / math.sqrt(self.feature_size * self.feature_size*16)))
	...
	'''
        ...
        # use outputs
        outputs = tf.reshape(outputs, [self.views, self.hidden_size])

        #view-level attention
        L1_S2_Similarity = tf.nn.softmax(tf.matmul(tf.matmul(outputs, Matrix_L1_S2),
        ...
        view_sums = tf.reduce_sum(tf.multiply(view_similarity, outputs), axis=0, keep_dims=True)

        model_feature = tf.nn.l2_normalize(view_sums, 1)
        ...
        for iter in range(max_iters):
            ...
            feed_dict = {self.net.data: blobs['data'], self.net.im_info: blobs['im_info'], self.net.keep_prob: 1.0,
                         ...
                         self.net10.data: blobs10['data'], self.net10.im_info: blobs10['im_info'],
                         self.net10.keep_prob: 1.0,
                         self.net11.data: blobs11['data'], self.net11.im_info: blobs11['im_info'],
